...
def verify_RH_list(zeros, heights, value, maximum, tail):
    ...
    t0_contribution = value(Decimal("1"), t0)       #find the amount that t0 adds to the sum
    ...
    for zero in zeros:      #loop through all the zeros given
        ...
        while (sum + t0_contribution) >= maximum:       #see if the sum plus the contribution from t0 exceeds the maximum
            ...
            t0_contribution = value(Decimal("1"), t0)       #find contribution of the next height
...
def main():
    ...
    for sum in sums:
        # sum = Decimal("0.02309570896612103381")
        ...
        for word in result_strings:
            print("{:<50}".format(word), end="")
        print()
        for result in results:
            for number in result:
                print("{:<50}".format(number), end="")
            print()
        print()
